...
while not app.need_exit():
    ...
    if status == 0:  # Selecting target
        ...
        if touch_status[2]:
            print(f"touch_status={touch_status} btn={[disp.width() - 100, disp.height() - 60, 100, 60]}")
            if is_in_button(touch_status[0], touch_status[1], [disp.width() - 100, disp.height() - 60, 100, 60]):
                status = 0
