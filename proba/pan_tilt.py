...
def change_baud_rate(scs_id, baud_rate):
    tx_res = packetHandler.write1ByteTxOnly(portHandler, scs_id, ADDR_SCS_LOCK, 0)
    raise_tx(tx_res)

    tx_res = packetHandler.write1ByteTxOnly(portHandler, scs_id, ADDR_SCS_BAUD_RATE, baud_rate)
    raise_tx(tx_res)

    tx_res = packetHandler.write1ByteTxOnly(portHandler, scs_id, ADDR_SCS_LOCK, 1)
    raise_tx(tx_res)
...
if __name__ == "__main__":
    ...
    ping(PAN_ID)
    #change_baud_rate(PAN_ID,1)
    ping(TILT_ID)
    #change_baud_rate(TILT_ID,1)
